...
3. **Google Chat.** The public handler is `chat/main.py` on Cloud Run service `chat` in `us-east1` at `https://chat.lab5.ca`. DNS for that host is a Cloudflare DNS-only CNAME from `chat.lab5.ca` to `ghs.googlehosted.com`. This repository does not manage that zone. `gmake chat-deploy` builds `chat/`, pushes the handler image, and applies Terraform. `gmake chat` is the laptop path. It exports `GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`, and `REASONING_ENGINE` from `infra/outputs.json` when those variables are unset, then runs `chat/main.py` on `127.0.0.1` and publishes that process with cloudflared. The process reads those three variables from the environment. Paste the `trycloudflare.com` HTTPS URL into the Chat app HTTP endpoint. The handler listens for a `MESSAGE`. It calls `reasoningEngines/{id}:streamQuery` with class method `async_stream_query`, and posts the model text in that thread. `user_id` is the Chat user. `session_id` is the space plus the thread. The handler holds no policy text.
